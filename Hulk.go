...
/*
 fuck love DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
...
func main() {
	...
	if version {
		fmt.Println("fucklove", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("-- fucklove Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- fucklove Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
